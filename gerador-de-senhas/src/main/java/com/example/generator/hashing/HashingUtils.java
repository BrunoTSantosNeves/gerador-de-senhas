...
public class HashingUtils {
    ...
    // Número de iterações para o algoritmo PBKDF2 (quanto maior, mais seguro e mais lento)
    private static final int PBKDF2_ITERATIONS = 10000;
    
    // Comprimento da chave gerada pelo PBKDF2 (256 bits = 32 bytes)
    private static final int PBKDF2_KEY_LENGTH = 256;
    
    // Gerador de números aleatórios seguro para criação de salt
    ...
    public static boolean verifyPassword(String password, String hashedPassword, HashAlgorithm algorithm) {
        switch (algorithm) {
            ...
            case BCRYPT:
                return BCrypt.checkpw(password, hashedPassword); // O próprio BCrypt já verifica o hash internamente
            case SHA256:
                return generateSHA256Hash(password).equals(hashedPassword); // Compara diretamente o hash gerado com o armazenado
            default:
                throw new IllegalArgumentException("Algoritmo de hash não suportado.");
        }
    }
    ...
    /**
     * Gera um hash usando o algoritmo PBKDF2 com HMAC-SHA256.
     *
     * @param password A senha a ser protegida.
    ...
    private static String generatePBKDF2Hash(String password) {
        try {
            // Criando um salt aleatório de 16 bytes
            byte[] salt = new byte[16];
            SECURE_RANDOM.nextBytes(salt);

            // Criando uma chave baseada na senha e no salt
            PBEKeySpec spec = new PBEKeySpec(password.toCharArray(), salt, PBKDF2_ITERATIONS, PBKDF2_KEY_LENGTH);
            SecretKeyFactory skf = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256");
            byte[] hash = skf.generateSecret(spec).getEncoded();

            // Retorna salt e hash concatenados e codificados em Base64
    ...
    private static boolean verifyPBKDF2(String password, String storedHash) {
        try {
            ...
            // Gera um hash com a senha fornecida e o salt extraído
            PBEKeySpec spec = new PBEKeySpec(password.toCharArray(), salt, PBKDF2_ITERATIONS, PBKDF2_KEY_LENGTH);
            SecretKeyFactory skf = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256");
    ...
    private static String generateBCryptHash(String password) {
        return BCrypt.hashpw(password, BCrypt.gensalt()); // Gera automaticamente um salt e aplica o hash
    }
